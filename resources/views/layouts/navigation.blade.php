@php
$isAdmin = Auth::guard('admin')->check() && !Auth::guard('borrower')->check();
$isBorrower = Auth::guard('borrower')->check();
$authUser = $isAdmin
? Auth::guard('admin')->user()
: ($isBorrower ? Auth::guard('borrower')->user() : null);
@endphp
...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-200 shadow-sm">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('admin.history.index')" :active="request()->routeIs('admin.history.*')" class="flex items-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <div class="flex items-center bg-gray-50 rounded-full pl-4 pr-1 py-1 border border-gray-100">
                    ...
                    @if($isAdmin)
                    <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider mr-3 bg-indigo-100 text-indigo-700">
                        Admin
                    </span>
                    @else
                    <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider mr-3 bg-emerald-100 text-emerald-700">
                        Faculty
                    </span>
                    @endif
                    ...
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-3 py-1.5 border border-transparent text-sm leading-4 font-semibold rounded-full text-gray-700 bg-white hover:text-indigo-600 shadow-sm transition ease-in-out duration-150">
                                <div>{{ $authUser?->full_name ?? 'Guest' }}</div>
                                <div class="ms-1">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-gray-50 border-t border-gray-200">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4 font-semibold text-sm text-gray-800">
